...
# Buscar el mínimo y máximo track_voxel_ratio
min_idx = param_df['track_voxel_ratio'].idxmin()
max_idx = param_df['track_voxel_ratio'].idxmax()

runs_extremos = {
    'min_trackratio': (int(param_df.loc[min_idx, 'run']), param_df.loc[min_idx, 'track_voxel_ratio']),
    'max_trackratio': (int(param_df.loc[max_idx, 'run']), param_df.loc[max_idx, 'track_voxel_ratio'])
}
...
for etiqueta, (run, valor_ratio) in runs_extremos.items():
    ...
    matrices[etiqueta] = (matriz, run, valor_ratio)
...
for etiqueta, (matriz, run, valor_ratio) in matrices.items():
    ...
    titulo = f"{etiqueta_formateada} track_voxel_ratio ({valor_ratio:.2f})"
    plt.title(titulo)
    ...
    nombre_salida = f"heatmap_{etiqueta}_trackratio_run{run:02d}.png"
    plt.tight_layout()
