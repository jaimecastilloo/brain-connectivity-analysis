...
# Buscar el mínimo y máximo fa_threshold
min_idx = param_df['fa_threshold'].idxmin()
max_idx = param_df['fa_threshold'].idxmax()

runs_extremos = {
    'min_fa': (int(param_df.loc[min_idx, 'run']), param_df.loc[min_idx, 'fa_threshold']),
    'max_fa': (int(param_df.loc[max_idx, 'run']), param_df.loc[max_idx, 'fa_threshold'])
}
...
for etiqueta, (run, valor_fa) in runs_extremos.items():
    ...
    matrices[etiqueta] = (matriz, run, valor_fa)
...
for etiqueta, (matriz, run, valor_fa) in matrices.items():
    ...
    titulo = f"{etiqueta_formateada} fa_threshold ({valor_fa:.3f})"
    plt.title(titulo)
    ...
    nombre_salida = f"heatmap_{etiqueta}_fa_run{run:02d}.png"
    plt.tight_layout()
